...
export default function SuperAdminHomePage({ language }: Props) {
  ...
  const copy = {
    ...
    attentionNow: isBg ? 'Нужно внимание сега' : 'Needs attention now',
    activityFeed: isBg ? 'Оперативен изглед' : 'Operational view',
    quickActions: isBg ? 'Бързи действия' : 'Quick actions',
    recentNotifications: isBg ? 'Последни известия' : 'Recent notifications',
    pendingRequests: isBg ? 'Чакащи заявки' : 'Pending requests',
    openTickets: isBg ? 'Отворени тикети' : 'Open tickets',
    waitingUsers: isBg ? 'Чакат потребител' : 'Waiting on user',
    unreadNotifications: isBg ? 'Непрочетени известия' : 'Unread notifications',
    latestRequests: isBg ? 'Последни заявки' : 'Latest requests',
    supportQueue: isBg ? 'Опашка за поддръжка' : 'Support queue',
    emptyRequests: isBg ? 'Няма чакащи заявки за одобрение.' : 'There are no pending requests to approve.',
    emptySupport: isBg ? 'Няма активни тикети в момента.' : 'There are no active tickets right now.',
    emptyNotifications: isBg ? 'Няма нови известия.' : 'There are no new notifications.',
    noAddress: isBg ? 'Без адрес' : 'No address',
    noPreview: isBg ? 'Няма преглед на съобщение.' : 'No message preview.',
    statusOpen: isBg ? 'Отворен' : 'Open',
    statusWaiting: isBg ? 'Чака потребител' : 'Waiting user',
    statusClosed: isBg ? 'Затворен' : 'Closed',
    sourcePublic: isBg ? 'Уеб/имейл' : 'Web/email',
    sourceDashboard: isBg ? 'Платформа' : 'Platform',
    sourceEmail: isBg ? 'Имейл' : 'Email',
    requestsAction: isBg ? 'Към заявките' : 'Open requests',
    supportAction: isBg ? 'Към поддръжката' : 'Open support',
    notificationsAction: isBg ? 'Известия' : 'Open notifications',
    requestContact: isBg ? 'Контакт' : 'Contact',
    viewAll: isBg ? 'Виж всички' : 'View all',
    error: isBg ? 'Неуспешно зареждане на началното табло.' : 'Failed to load the home dashboard.',
  }

  const loadHomeData = useCallback(
    async (options?: { silent?: boolean; showRefreshIndicator?: boolean }) => {
      const isSilent = Boolean(options?.silent)
      const showRefreshIndicator = Boolean(options?.showRefreshIndicator)

      if (!isSilent) setIsLoading(true)
      if (showRefreshIndicator) setIsManualRefreshing(true)
      ...
      try {
        ...
        if (!isSilent) pushToast('error', copy.error)
      } finally {
        if (!isSilent) setIsLoading(false)
        if (showRefreshIndicator) setIsManualRefreshing(false)
      }
    },
  ...
  const quickActions = [
    {
      ...
      description: isBg ? 'Преглед и одобряване на нови автошколи.' : 'Review and approve new driving schools.',
      href: '/dashboard/superadmin/requests',
      icon: <UserRoundPlus className="h-4 w-4" />,
    },
    {
      title: copy.supportAction,
      description: isBg ? 'Обработване на текущите тикети и разговори.' : 'Handle active tickets and conversations.',
      href: '/dashboard/superadmin/support',
      icon: <Ticket className="h-4 w-4" />,
    },
    {
      title: copy.notificationsAction,
      description: isBg ? 'Преглед на известията, които системата е изпратила.' : 'Review the notifications emitted by the system.',
      href: '/dashboard/superadmin/notifications',
        ...
        <div className="rounded-xl border border-base-300/80 bg-base-100/90 p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-4 text-xl font-semibold tracking-tight text-base-content sm:text-2xl">
                {copy.title}
              </h2>
          ...
          <div className="mt-5 space-y-3">
            {quickActions.map((action) => (
              <Link
                key={action.href}
                to={action.href}
                className="flex items-start gap-3 rounded-xl border border-base-300/80 bg-base-100 p-4 transition-colors hover:bg-base-200/60"
              >
                <span className="mt-0.5 inline-flex rounded-lg bg-base-200 p-2 text-base-content/75">
                  {action.icon}
                </span>
          ...
          <div className="mt-5 rounded-xl border border-base-300/80 bg-base-100 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-base-content/60">
                  {isBg ? 'Последните системни събития към вашия профил.' : 'Latest system events delivered to your profile.'}
                </p>
              </div>
            ...
            <div className="mt-4 space-y-2">
              {notificationRows.length === 0 ? (
                <p className="rounded-xl border border-dashed border-base-300 px-3 py-4 text-sm text-base-content/55">
                  ...
                  <div key={item.id} className="rounded-xl border border-base-300/80 bg-base-100 px-3 py-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <span className={`badge badge-sm ${item.read ? 'badge-ghost' : 'badge-primary'}`}>
                        {getNotificationLabel(item, language)}
                      </span>
                      <span className="text-[11px] text-base-content/45">{formatDate(item.createdAt, language)}</span>
                    </div>
                    <p className="mt-2 line-clamp-2 text-sm text-base-content/75">
                      {item.title || item.body || (isBg ? 'Системно известие' : 'System notification')}
                    </p>
                  </div>
                ))
              )}
        ...
        <div className="rounded-xl border border-base-300/80 bg-base-100/90 p-4 shadow-sm sm:p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="tabs tabs-boxed border border-base-content/10 bg-base-200/70 p-1">
              <button type="button" className={`tab ${activePanel === 'requests' ? 'tab-active' : ''}`} onClick={() => setActivePanel('requests')}>
                {copy.pendingRequests}
              </button>
              <button type="button" className={`tab ${activePanel === 'support' ? 'tab-active' : ''}`} onClick={() => setActivePanel('support')}>
                {copy.openTickets}
              </button>
              <button type="button" className={`tab ${activePanel === 'notifications' ? 'tab-active' : ''}`} onClick={() => setActivePanel('notifications')}>
                    ...
                    <article key={request.id} className="rounded-xl border border-base-300/80 bg-base-100 p-4 transition-colors hover:bg-base-200/60">
                      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="shrink-0 text-left md:text-right">
                          ...
                          <Link to="/dashboard/superadmin/requests" className="btn btn-ghost btn-sm mt-2 rounded-xl">
                            {copy.requestsAction}
                          </Link>
                        </div>
                      </div>
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="text-base font-semibold text-base-content">
                              {thread.ticketSubject?.trim() || `Ticket #${thread.id.slice(0, 8)}`}
                            </p>
                            <span className={`badge badge-sm ${thread.status === 'OPEN' ? 'badge-success' : 'badge-warning'}`}>
                              {statusLabel(thread.status)}
                            </span>
                          </div>
                          <p className="mt-1 text-sm text-base-content/65">
                            {thread.requesterName} · {sourceLabel(thread.source)}
                          </p>
                          <p className="mt-2 line-clamp-2 text-sm text-base-content/60">
                            {thread.latestMessagePreview || copy.noPreview}
                          </p>
                        </div>
                        <div className="shrink-0 text-left md:text-right">
                          <p className="text-xs font-medium text-base-content/45">{formatDate(thread.updatedAt, language)}</p>
                          <Link to="/dashboard/superadmin/support" className="btn btn-ghost btn-sm mt-2 rounded-xl">
                            {copy.supportAction}
                          </Link>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {!item.read ? <span className="text-xs font-medium text-primary">{isBg ? 'Ново' : 'New'}</span> : null}
                          </div>
                          <p className="mt-2 text-sm font-medium text-base-content">
                            {item.title || (isBg ? 'Системно известие' : 'System notification')}
                          </p>
                        ...
                        <div className="shrink-0 text-left md:text-right">
                          ...
                          <Link to="/dashboard/superadmin/notifications" className="btn btn-ghost btn-sm mt-2 rounded-xl">
